...
import { TRemoteAccount } from "../../api/services";
import { TPortfolioTabAccount } from "../../widgets/portfolio/types";
...
/**
 * @description sync remote and local accounts
 */
export const syncAccounts = (
    localAccounts: TCryptoAccount[],
    remoteAcounts: TRemoteAccount[]
): TCryptoAccount[] => {
    const syncedAccounts = remoteAccountsAsCryptoAccounts(remoteAcounts);
    removeDuplicateAccounts(localAccounts).forEach((localAcc) => {
        if (!accountExists(syncedAccounts, localAcc)) {
            syncedAccounts.push(localAcc);
        }
    });
