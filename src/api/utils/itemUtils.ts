...
// 90% of the scroll height of the element
const SCROLL_ITEM_FRACTION = 0.1;

/**
 * Gets the scroll position of the target and compare it to the scrollable height.
 *
 * @param target the target element to scroll to
 * @returns true if the target is within the scrollable height of the scroll position.
 */
export const canMoveToNextItem = (target: HTMLElement): boolean => {
    return target.scrollTop >= target.scrollHeight * SCROLL_ITEM_FRACTION;
};
