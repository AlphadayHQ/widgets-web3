...
import { TGasPrices } from "../../api/types";
...
export type TZapperNftAsset = {
    ...
    token: {
        name: string;
        tokenId: number; // number in string format
        ...
        collection: {
            ...
            floorPriceEth: number; // number in string format
            logoImageUrl: string;
            openseaId: string;
        };
    };
...
export const computeNftAssetTotal: (a: TZapperNftAsset[]) => number = (a) => {
    const total: number = a.reduce(
        (prev, cur) =>
            cur.token.estimatedValueEth
                ? prev + parseFloat(cur.token.estimatedValueEth)
                : prev,
        0
    );
    return total;
                ...
                await new Promise<void>((resolve, reject) => {
                    source.addEventListener("balance", (e) => {
                        try {
                            ...
                            const { wallet } = res?.balance;

                            if (!wallet || !address) return;

                            const assets: TZapperAsset[] = [];

                            Object.values(wallet).forEach((token) => {
                                assets.push(token as TZapperAsset);
                            });
